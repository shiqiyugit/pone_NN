...
class DataGenerator(keras.utils.Sequence):
    ...
    """
    def __init__(self, hdf5_file, labels=None, maxlen=None, use_log_energy=False, use_weights=False, normal=[], batch_size=256):#512
        ...
        if labels is None:
            ...
            for k in self.label_keys:
                if (k not in available_keys):
                    if   k == "dir_x": gen_dx = True
                    elif k == "dir_y": gen_dy = True
                    elif k == "dir_z": gen_dz = True
                    else:
            ...
            if gen_dx or gen_dy or gen_dz:
                ...
                for k in available_keys:
                    ...
                    if k == "dir_x": dx = self.f["labels"]["dir_x"]
                    if k == "dir_y": dy = self.f["labels"]["dir_y"]
                    if k == "dir_z": dz = self.f["labels"]["dir_z"]
                ...
                if gen_dx: dx, dy, dz = self._to_dxdydz(zen[:], azi[:])
                if gen_dx: self.labels["dir_x"] = dx
                if gen_dy: self.labels["dir_y"] = dy
                if gen_dz: self.labels["dir_z"] = dz
    ...
    def __getitem__(self, index):
        ...
        dom_index    = pad_sequences(dom_index,    dtype=dom_index[0].dtype,    maxlen=self.max_hits)
        pulse_time   = pad_sequences(pulse_time,   dtype=pulse_time[0].dtype,   maxlen=self.max_hits)
        pulse_charge = pad_sequences(pulse_charge, dtype=pulse_charge[0].dtype, maxlen=self.max_hits)
