...
def plot_1dhist(true, predicted, minimum, maximum, quantity, weights, gen_filename="path/save_folder/", density=True, l1="True", l2="Predicted"):
    plt.figure()
    plt.title("Predicted vs. True " + strip_units(quantity))
    plt.xlabel(quantity)
    if quantity == "Energy [GeV]":
        plt.yscale("log")
    bins=100
    _, bins, _=plt.hist(true, bins=bins, range=[minimum,maximum], histtype="step", weights=weights, label=l1, density=density)
    plt.hist(predicted, bins=bins, histtype="step", weights=weights, label=l2, density=density)
    plt.legend(loc="best")
...
def plot_error_vs_reco(true, predicted, reco, minimum, maximum, quantity, quantity2=0 , x=0, gen_filename="path/save_folder/"):
    ...
    if quantity == "Energy [GeV]":
        fractional_errors = ((predicted-true)/true)*100. # in percent
        fractional_errors_reco = ((reco-true)/true)*100.
    elif quantity == "Azimuth [degrees]":
        fractional_errors = np.array([(predicted[i]-true[i]) if math.fabs((predicted[i]-true[i])) < 180 else ((predicted[i]-true[i]-360) if predicted[i] > true[i] else (predicted[i]-true[i]+360)) for i in range(len(true))])
        fractional_errors_reco = np.array([(reco[i]-true[i]) if math.fabs((reco[i]-true[i])) < 180 else ((reco[i]-true[i]-360) if reco[i] > true[i] else (reco[i]-true[i]+360)) for i in range(len(true))])
    elif quantity == "Zenith [degrees]":
        fractional_errors = predicted-true
        fractional_errors_reco = reco-true
    else:
        fractional_errors = predicted
        fractional_errors_reco = reco

    percentile_in_peak = 68.27
    left_tail_percentile  = (100.-percentile_in_peak)/2
    right_tail_percentile = 100.-left_tail_percentile

    if quantity2 == "Energy [GeV]":
        ranges  = np.logspace(math.log10(minimum), math.log10(maximum))
    else:
        ranges  = np.linspace(minimum, maximum, num=8)
    centers = (ranges[1:] + ranges[:-1])/2.
    ...
    for i in range(len(ranges)-1):
        ...
        lower_lim = upper_lim=median=lower_lim_reco=upper_lim_reco=median_reco=0
...
#            cut_reco = (x_reco >= val_from) & (x_reco < val_to)
            lower_lim_reco = np.percentile(fractional_errors_reco[cut], left_tail_percentile)
            upper_lim_reco = np.percentile(fractional_errors_reco[cut], right_tail_percentile)
            median_reco = np.percentile(fractional_errors_reco[cut], 50.)
        except:
            print("lower_lim, upper_lim, median, lower_lim_reco, upper_lim_reco, median_reco:", lower_lim, upper_lim, median, lower_lim_reco, upper_lim_reco, median_reco)
    ...
    plt.figure()
    plt.errorbar(centers, medians, yerr=[medians-err_from, err_to-medians], fmt='o',label="RNN", capsize=5)
    plt.errorbar(centers, medians_reco, yerr=[medians_reco-err_from_reco, err_to_reco-medians_reco], fmt='o', label="Linefit", capsize=5)
    plt.plot([minimum,maximum], [0,0], color='k')
...
#    plt.ylim(0,10)
    ...
    if quantity == "Energy [GeV]":
        ...
        plt.ylabel(strip_units(quantity) + " Error" + get_units(quantity))
    plt.legend(loc="best")
    imgname = gen_filename + file_abbrev(quantity) + '_' + file_abbrev(quantity2) + "_err_comp.png"
    plt.savefig(imgname)
